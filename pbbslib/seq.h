...
#ifdef CONCEPTS
template <typename T> concept bool Seq = requires(T t, size_t u) {
...
template <typename T> concept bool Range = Seq<T> &&requires(T t, size_t u) {
    { t[u] }
    ->typename T::value_type &;
    typename T::iterator;
};
...
template <typename Iterator> struct range {
  public:
    ...
    range<std::reverse_iterator<value_type *>> rslice(size_t ss,
                                                      size_t ee) const {
...
template <class Iter> range<Iter> make_range(Iter s, Iter e) {
    return range<Iter>(s, e);
}

template <typename T, typename F> struct delayed_sequence {
    using value_type = T;
...
template <class T, class F> delayed_sequence<T, F> delayed_seq(size_t n, F f) {
...
template <typename T> struct sequence {
  public:
    ...
    sequence(const sequence &a) : n(a.n) {
        copy_here(a.s, a.n);
        if (check_copy)
            std::cout << "copy constructor: len: " << a.n
                      << " sizeof: " << sizeof(T) << std::endl;
    }
    ...
    sequence &operator=(const sequence &a) {
        ...
        if (check_copy)
            std::cout << "copy assignment: len: " << a.n
                      << " sizeof: " << sizeof(T) << std::endl;
        return *this;
    }
    ...
    static sequence<T> no_init(const size_t n) {
        ...
        // if (n > 1000000000) std::cout << "make no init: " << r.s <<
        // std::endl;
        r.n = n;
        return r;
    };
    ...
    sequence(const size_t _n, Func f)
        : s(pbbs::new_array_no_init<T>(_n)), n(_n) {
    ...
    template <typename Iter> sequence(range<Iter> a) {
        copy_here(a.begin(), a.size());
    }

    template <class F> sequence(delayed_sequence<T, F> a) {
        copy_here(a, a.size());
    }
  ...
  private:
    template <class Seq> void copy_here(Seq const &a, size_t an) {
        n = an;
...
template <class Iter> bool slice_eq(range<Iter> a, range<Iter> b) {
    return a.begin() == b.begin();
}

template <class SeqA, class SeqB> bool slice_eq(SeqA a, SeqB b) {
    return false;
}
