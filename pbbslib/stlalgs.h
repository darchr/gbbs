...
template <class IntegerPred> size_t count_if_index(size_t n, IntegerPred p) {
    auto BS =
...
size_t find_if_index(size_t n, IntegerPred p, size_t granularity = 1000) {
    ...
    while (start < n) {
        ...
        size_t i =
            pbbs::reduce(delayed_seq<size_t>(end - start, f), minm<size_t>());
        if (i < n)
            return i;
        start += granularity;
        granularity *= 2;
...
template <class Seq, class T> size_t count(Seq const &S, T const &value) {
    return count_if_index(S.size(), [&](size_t i) { return S[i] == value; });
}

template <class Seq, class UnaryPred> bool all_of(Seq const &S, UnaryPred p) {
    return count_if(S, p) == S.size();
}

template <class Seq, class UnaryPred> bool any_of(Seq const &S, UnaryPred p) {
    return count_if(S, p) > 1;
}

template <class Seq, class UnaryPred> bool none_of(Seq const &S, UnaryPred p) {
...
template <class Seq, class T> size_t find(Seq const &S, T const &value) {
...
template <class Seq> sequence<typename Seq::value_type> reverse(Seq const &S) {
    size_t n = S.size();
...
template <class Seq, class Compare> bool is_sorted(Seq const &S, Compare comp) {
...
size_t is_sorted_until(Seq const &S, Compare comp) {
    return find_if_index(S.size() - 1, [&](size_t i) { f(S[i + 1], S[i]); }) +
           1;
}
...
//
// Two elements are considered duplicates if neither compares less than the
// other according to `less`. The first such element is kept.
template <class Seq, class Compare>
