...
template <typename T> double t_tabulate(size_t n, bool check) {
    auto f = [](size_t i) { return i; };
    time(t, pbbs::sequence<T>(n, f););
    return t;
}

template <typename T> double t_map(size_t n, bool check) {
...
template <typename T> double t_reduce_add(size_t n, bool check) {
...
template <typename T> double t_scan_add(size_t n, bool check) {
    pbbs::sequence<T> In(n, (T)1);
    pbbs::sequence<T> Out;
    T sum;
...
template <typename T> double t_scan_add_seq(size_t n, bool check) {
    pbbs::sequence<T> In(n, (T)1);
    pbbs::sequence<T> Out(n, (T)0);
    time(
        t, T x = 0; for (size_t i = 0; i < n; i++) {
            Out[i] = x;
            x += In[i];
        });
    return t;
...
template <typename T> double t_pack(size_t n, bool check) {
...
template <typename T> double t_split3_old(size_t n, bool check) {
...
template <typename T> double t_split3(size_t n, bool check) {
    pbbs::random r(0);
...
template <typename T> double t_partition(size_t n, bool check) {
    pbbs::random r(0);
...
template <typename T> double t_gather(size_t n, bool check) {
    pbbs::random r(0);
    ...
    // note problem with prefetching since will go over end for last 4
    // iterations
    time(t, pbbs::sequence<T>(n - 4, f););
    return t;
}

template <typename T> double t_scatter(size_t n, bool check) {
    pbbs::random r(0);
    pbbs::sequence<T> out(n, (T)0);
    pbbs::sequence<T> idx(n, [&](size_t i) { return r.ith_rand(i) % n; });
    auto f = [&](size_t i) {
...
template <typename T> double t_write_add(size_t n, bool check) {
    pbbs::random r(0);
    // pbbs::sequence<T> out(n, (T) 0);
    pbbs::sequence<std::atomic<T>> out(n);
    pbbs::parallel_for(0, n,
                       [&](size_t i) { std::atomic_init(&out[i], (T)0); });
    pbbs::sequence<T> idx(n, [&](size_t i) { return r.ith_rand(i) % n; });
    auto f = [&](size_t i) {
...
template <typename T> double t_write_min(size_t n, bool check) {
    pbbs::random r(0);
    pbbs::sequence<std::atomic<T>> out(n);
    pbbs::parallel_for(0, n,
                       [&](size_t i) { std::atomic_init(&out[i], (T)n); });
...
template <typename T> double t_shuffle(size_t n, bool check) {
    pbbs::sequence<T> in(n, [&](size_t i) { return i; });
...
template <typename T> double t_histogram(size_t n, bool check) {
    pbbs::random r(0);
    pbbs::sequence<T> in(n, [&](size_t i) { return r.ith_rand(i) % n; });
    pbbs::sequence<T> out;
    time(t, out = pbbs::histogram<T>(in, n););
    if (check)
        check_histogram(in, out);
    return t;
}

template <typename T> double t_histogram_few(size_t n, bool check) {
    pbbs::random r(0);
...
template <typename T> double t_histogram_same(size_t n, bool check) {
...
bool check_sort(pbbs::sequence<T> const &in, pbbs::sequence<T> const &out,
    ...
    if (err_loc != n) {
        std::cout << "ERROR in " << sort_name << " at location " << err_loc
                  << std::endl;
        return false;
    }
    return true;
...
template <typename T> double t_sort(size_t n, bool check) {
    pbbs::random r(0);
    pbbs::sequence<T> in(n, [&](size_t i) { return r.ith_rand(i) % n; });
    pbbs::sequence<T> out;
    time(t, out = pbbs::sample_sort(in, std::less<T>()););
    if (check)
        check_sort(in, out, std::less<T>(), "sample sort");
    return t;
}

// no check since it is used for the sort for checking, and hence
// checked against the other sorts
template <typename T> double t_merge_sort(size_t n, bool check) {
    pbbs::random r(0);
    pbbs::sequence<T> in(n, [&](size_t i) { return r.ith_rand(i) % n; });
    pbbs::sequence<T> out;
...
template <typename T> double t_quicksort(size_t n, bool check) {
    pbbs::random r(0);
    pbbs::sequence<T> in(n, [&](size_t i) { return r.ith_rand(i) % n; });
    ...
    if (check)
        check_sort(copy, in, std::less<T>(), "quicksort");
    return t;
}

template <typename T> double t_count_sort_bits(size_t n, size_t bits) {
    pbbs::random r(0);
...
template <typename T> double t_count_sort_8(size_t n, bool check) {
    return t_count_sort_bits<T>(n, 8);
}

template <typename T> double t_count_sort_2(size_t n, bool check) {
    return t_count_sort_bits<T>(n, 2);
}

template <typename T> double t_collect_reduce_pair_dense(size_t n, bool check) {
    ...
    time(t,
         out = pbbs::collect_reduce(S, get_key, get_val, pbbs::addm<T>(), n););
    if (check)
        check_histogram(
            pbbs::sequence<T>(n, [&](size_t i) { return S[i].first; }), out);
    return t;
}
...
template <typename T> double t_collect_reduce_8(size_t n, bool check) {
    using par = std::pair<T, T>;
    pbbs::random r(0);
...
template <typename T> double t_integer_sort_pair(size_t n, bool check) {
    using par = std::pair<T, T>;
    pbbs::random r(0);
    size_t bits = sizeof(T) * 8;
    pbbs::sequence<par> S(
        n, [&](size_t i) -> par { return par(r.ith_rand(i), i); });
    pbbs::sequence<par> R;
    ...
    if (check)
        check_sort(S, R, less, "integer sort pair");
    return t;
}

template <typename T> double t_integer_sort(size_t n, bool check) {
    pbbs::random r(0);
    size_t bits = sizeof(T) * 8;
    ...
    if (check)
        check_sort(S, R, std::less<T>(), "integer sort");
    return t;
}

double t_integer_sort_128(size_t n, bool check);

template <typename T> double t_merge(size_t n, bool check) {
...
template <typename T> double t_remove_duplicates(size_t n, bool check) {
    pbbs::random r(0);
...
template <typename T> double t_bag(size_t n, bool check) {
    using TB = pbbs::bag<T>;
    TB::init();
...
double t_mat_vec_mult(size_t n, bool check) {
    ...
    size_t degree = 5;
    size_t m = degree * n;
    pbbs::sequence<s_size_t> starts(n + 1,
                                    [&](size_t i) { return degree * i; });
    pbbs::sequence<s_size_t> columns(
        m, [&](size_t i) { return r.ith_rand(i) % n; });
...
template <typename T> double t_range_min(size_t n, bool check) {
