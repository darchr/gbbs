...
int main(int argc, char *argv[]) {
    ...
    string fname =
        "/home/guyb/tmp/pbbsbench/testData/sequenceData/data/wikisamp.xml";
    // string fname = "/ssd0/text/HG18.fasta";
    // string fname = "foo";
    // string fname = "bar";
    sequence<char> teststr = char_seq_from_file(fname);
    // auto r = tokens(teststr, [&] (char c) {return c != 'N';});
    // uint max = reduce(sequence<uint>(r.size(), [&] (size_t i) {return
    // r[i].size();}), maxm<uint>()); cout << "max: " << max << std::endl;
    // sequence<char> out = filter(teststr, [&] (char c) {return (c == 'A') || c
    // == 'C' || c == 'T' || c == 'G' || c == 'M' || c == 'N';});
    ...
    // for (size_t i = 0; i < n; i++) std::cout << R[i].location << ", " <<
    // R[i].value << ", " << R[i].parent << std::endl;
}
