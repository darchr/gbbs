...
template <typename Job> struct Deque {
    ...
    void push_bottom(Job *job) {
        ...
        local_bot += 1;
        if (local_bot == q_size) {
            std::cout << "internal error: scheduler queue overflow"
                      << std::endl;
            abort();
        }
...
template <typename Job> struct scheduler {
  public:
    int num_threads;
    ...
    scheduler() {
        ...
        for (int i = 1; i < num_threads; i++) {
            ...
            CPU_ZERO(&cpuset);
            CPU_SET(i, &cpuset);
            pthread_setaffinity_np(spawned_threads[i - 1].native_handle(),
    ...
    template <typename F> void wait(F finished, bool conservative = false) {
  ...
  private:
    ...
    template <typename F> void start(F finished) {
        while (1) {
    ...
    template <typename F> Job *get_job(F finished) {
        if (finished())
            return NULL;
        Job *job = try_pop();
        if (job)
        ...
        while (1) {
            ...
            std::this_thread::sleep_for(
                std::chrono::nanoseconds(num_deques * 100));
        }
    }
...
template <typename T> thread_local int scheduler<T>::thread_id = 0;

#ifdef SAGE
template <typename T> thread_local int scheduler<T>::numa_node = 0;
#endif
...
struct fork_join_scheduler {
  public:
    ...
    template <typename F> int get_granularity(size_t start, size_t end, F f) {
        size_t done = 0;
        size_t size = 1;
        int ticks;
        do {
    ...
    void parfor(size_t start, size_t end, F f, size_t granularity = 0,
        ...
        if (granularity == 0) {
            ...
            granularity =
                std::max(done, (end - start) / (128 * sched->num_threads));
  ...
  private:
    ...
    void parfor_(size_t start, size_t end, F f, size_t granularity,
                 bool conservative) {
        // std::cout << "Start: " << start << " End: " << end << " Granularity:
        // " << granularity << std::endl;
