...
void write_graph_bytepd_amortized_format(Graph &GA, std::ofstream &out,
    ...
    //      xors[i] = xr;
    //    }
    //    std::cout << "input graph: output red = " << pbbslib::reduce_xor(xors)
    //    << std::endl;
    //
    ...
    //      xors[i] = pri;
    //    }
    //    std::cout << "input graph: priorities = " << pbbslib::reduce_add(xors)
    //    << std::endl; std::cout << "input graph: self-loops = " <<
    //    pbbslib::reduce_add(self_arr) << std::endl;
    ...
    par_for(0, n, [&](size_t i) {
        ...
        if (deg > 0) {
            ...
            //        sizeof(uintE)); current_offset += sizeof(uintE) +
            //        (num_blocks-1)*sizeof(uintE);
            ...
            //              *prev_block_deg = PARALLEL_DEGREE; // full block;
            //              write prev block's degree block_offsets[cur_block-1]
            //              = current_offset; // write start of this block
            //            }
            //            prev_block_deg = (uintE*)(edgeArray + current_offset);
            //            *prev_block_deg = 0;
            //            current_offset += sizeof(uintE);
            //            current_offset = compressFirstEdge(edgeArray,
            //            current_offset, u, v); current_offset =
            //            compressWeight<W>(edgeArray, current_offset, w);
            //            cur_block++;
            //          } else {
            //            current_offset = compressEdge(edgeArray,
            //            current_offset, v - last_ngh); current_offset =
            //            compressWeight<W>(edgeArray, current_offset, w);
            //          }
            //          last_ngh = v;
            //          proc++;
            ...
            //
            //        assert(current_offset == (byte_offsets[i+1] -
            //        byte_offsets[i]));
    ...
    //      xors[i] = xr;
    //    }
    //    std::cout << "output graph: output red = " <<
    //    pbbslib::reduce_xor(xors)
    //    << std::endl;
    ...
    //      xors[i] = pri;
    //    }
    //    std::cout << "output graph: priorities = " <<
    //    pbbslib::reduce_add(xors)
    //    << std::endl; std::cout << "output graph: self-loops = " <<
    //    pbbslib::reduce_add(self_arr) << std::endl;
...
template <class Graph> double converter(Graph &GA, commandLine P) {
