...
template <class I> struct wgh_it {
    uintE deg;
    I &it;
    uintE source;
...
template <class I, class GA> struct degree_wgh_it {
    uintE deg;
    I &it;
    uintE source;
    uintE src_degree;
    GA &G;
    degree_wgh_it(I &_it, uintE _src, GA &G)
        : it(_it), source(_src), deg(0), G(G) {
    ...
    tuple<uintE, int> cur() {
        ...
        int wgh = ngh_degree + src_degree; //(int)(((double)1)/((double)(ngh_degree
                                           //+ src_degree)));
        // uintE ind = pbbs::hash64(source ^ get<0>(cr));
        // int wgh = Choices[ind % (2 * max_weight)];
        return make_tuple(get<0>(cr), wgh);
    ...
    tuple<uintE, int> next() {
        ...
        int wgh =
            ngh_degree +
            src_degree; //(int)(((double)1)/((double)(ngh_degree +
                        //src_degree)));
                        //    uintE ind = pbbs::hash64(source ^ get<0>(nxt));
                        //    int wgh = Choices[ind % (2 * max_weight)];
        return make_tuple(get<0>(nxt), wgh);
...
template <class I, class GA> auto make_wgh_it(I &it, uintE source, GA &G) {
    ...
    parallel_for(
        ...
        [&](size_t i) {
            ...
            if (k != GA.V[i].getOutDegree()) {
                std::cout << "k = " << k << " deg = " << GA.V[i].getOutDegree()
                          << std::endl;
            }
...
template <class vertex> void Reencoder(graph<vertex> &GA, commandLine P) {
    auto outfile =
    ...
    if (encoding == "adj") {
        ...
        encodings::bytepd_amortized::writeWeightedBytePDA(GA, outfile,
                                                          symmetric);
    }
