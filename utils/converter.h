...
void write_graph_bytepd_format(Graph &GA, std::ofstream &out, bool symmetric,
    ...
    auto deg_im =
        pbbs::delayed_seq<size_t>(n, [&](size_t i) { return degrees[i]; });
    ...
    for (size_t i = 0; i < bs; i++) {
        ...
        if (start >= end)
            break;
        std::cout << "# writing vertices " << start << " to " << end
                  << std::endl;
        ...
        parallel_for(
            ...
            [&](size_t i) {
                ...
                if (deg > 0) {
                    auto it = GA.get_vertex(i).out_neighbors().get_iter();
                    size_t nbytes =
                        bytepd_amortized::sequentialCompressEdgeSet<W>(
                            edges.begin() + byte_offsets[i], 0, deg, (uintE)i,
                            it, PAR_DEGREE_TWO);
                    if (nbytes != (byte_offsets[i + 1] - byte_offsets[i])) {
                        std::cout << "# nbytes = " << nbytes << ". Should be: "
                                  << (byte_offsets[i + 1] - byte_offsets[i])
...
template <class Graph>
void write_graph_binary_format(Graph &GA, std::ofstream &out,
                               size_t n_batches = 4) {
    size_t n = GA.n;
    size_t m = GA.m;
    using W = typename Graph::weight_type;
...
template <class Graph> auto converter(Graph &GA, commandLine P) {
    ...
    if (encoding == "byte") {
        ...
        bytepd_amortized::write_graph_bytepd_amortized_format(GA, out,
                                                              symmetric);
