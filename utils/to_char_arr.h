...
template <class A, class B> inline int xToStringLen(std::pair<A, B> a) {
    return xToStringLen(a.first) + xToStringLen(a.second) + 1;
}
template <class A, class B> inline void xToString(char *s, std::pair<A, B> a) {
...
template <class S> sequence<char> arrayToString(S &A) {
    auto AA = A.begin();
    size_t n = A.size();
    auto L = pbbs::sequence<size_t>(
...
template <class S> void writeArrayToStream(std::ofstream &os, S &arr) {
    ...
    while (offset < n) {
        ...
        auto arr_slice =
            arr.slice(offset, offset + std::min(BSIZE, n - offset));
