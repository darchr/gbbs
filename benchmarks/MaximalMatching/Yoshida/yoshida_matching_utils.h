...
struct sorted_it {
    ...
    sorted_it(uintE *ngh_a, uintE *ngh_b, uintE deg_a, uintE deg_b, uintE id_a,
              uintE id_b)
        : ngh_a(ngh_a), ngh_b(ngh_b), deg_a(deg_a), deg_b(deg_b), id_a(id_a),
          id_b(id_b) {
        ct_a = 0;
        ct_b = 0;
        tot = deg_a + deg_b;
    ...
    /* specialize for now */
    auto lt(const uintE &a, const uintE &n_a, const uintE &b,
            const uintE &n_b) {
        assert(a < n_a);
        assert(b < n_b);
    ...
    std::pair<uintE, uintE> get_next() {
        if (ct_a < deg_a) {
            if (ct_b < deg_b) {
                ...
                uintE b = ngh_b[ct_b];
                auto pair_a =
                    std::make_pair(std::min(id_a, a), std::max(id_a, a));
                auto pair_b =
                    std::make_pair(std::min(id_b, b), std::max(id_b, b));
                auto ret = lt(pair_a.first, pair_a.second, pair_b.first,
                              pair_b.second);
                if (ret == pair_a) {
                    ct_a++;
                } else {
