...
ComputeStructuralSimilarities(symmetric_graph<VertexType, NoWeight> *graph) {
    ...
    using VertexSet = pbbslib::sparse_table<uintE, pbbslib::empty,
    ...
    parallel_for(0, graph->n,
                 [&graph, &adjacency_list](const size_t vertex_id) {
        ...
        if (u_id < v_id) {
            ...
            const size_t num_shared_neighbors{
                smaller_degree_vertex->out_neighbors().reduce(
                    is_shared_neighbor, addition_monoid)};

            // The neighborhoods we've computed are open neighborhoods -- since
            // structural similarity uses closed neighborhoods, we need to
            // adjust the numbers slightly, hence the `+ 1` and `+ 2` terms.
            similarities.insert(
