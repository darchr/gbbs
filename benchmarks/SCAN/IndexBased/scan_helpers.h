...
class NeighborOrder {
  public:
    ...
    //
    // The neighbor lists for each vertex in the graph must be sorted by
...
struct CoreThreshold {
    uintE vertex_id;
    // Maximum value of the SCAN parameter epsilon for which `vertex_id` is a
    // core vertex (given some fixed reference value for SCAN parameter mu).
    float threshold;
};
    ...
    similarities_by_source_ = pbbs::sequence<pbbs::range<EdgeSimilarity *>>(
        graph->n, [&](const size_t i) {
            return similarities_.slice(
                vertex_offsets[i], i + 1 == graph->n ? similarities_.size()
                                                     : vertex_offsets[i + 1]);
        });
