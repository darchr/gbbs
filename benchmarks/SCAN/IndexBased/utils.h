...
pbbs::sequence<typename std::remove_reference_t<Monoid>::T>
CollectReduce(const Seq &seq, Key_fn &&get_key, Value_fn &&get_value,
              Monoid &&reduce_fn, size_t num_keys) {
    using Value = typename std::remove_reference_t<Monoid>::T;
    pbbs::sequence<size_t> bucketed_indices{seq.size(),
        ...
        [&](const uintE v_id, const uintE neighbor_id, pbbslib::empty) {
            ...
            if (neighbor_cluster != kUnclustered &&
                !(candidate_cluster == UINT_E_MAX &&
                  pbbs::atomic_compare_and_swap(&candidate_cluster, UINT_E_MAX,
                                                neighbor_cluster)) &&
...
double Modularity(symmetric_graph<VertexTemplate, Weight> *graph,
    ...
    const size_t num_edges{
        graph->m}; // two times the number of undirected edges
    if (num_edges == 0) {
        return 0.0;
    }
    const size_t num_vertices{graph->n};
    const size_t num_clusters{
        1 + pbbslib::reduce_max(pbbs::delayed_seq<uintE>(
                num_vertices, [&](const size_t vertex_id) {
    ...
    if constexpr (std::is_same<Weight, pbbslib::empty>::value) {
        ...
        const double intracluster_edge_proportion{
            static_cast<double>(pbbslib::reduce_add(pbbs::delayed_seq<uintT>(
                ...
                [&](const size_t vertex_id) -> uintT {
                    ...
                    const auto is_same_cluster{[&](const uintE v_id,
                                                   const uintE ngh_id,
                                                   pbbslib::empty) {
        ...
        const auto degrees_split_result{pbbs::split_two(
            ...
            pbbs::delayed_seq<bool>(num_vertices, [&](const size_t i) {
        ...
        const pbbs::sequence<uintT> degrees_by_cluster{internal::CollectReduce(
            clusters_and_degrees.slice(num_unclustered_vertices,
                                       clusters_and_degrees.size()),
            [&](const std::pair<uintE, uintT> p) { return p.first; },
            [&](const std::pair<uintE, uintT> p) { return p.second; },
            pbbs::addm<uintT>{}, num_clusters)};
        ...
        const double intracluster_edge_proportion{
            pbbslib::reduce_add(pbbs::delayed_seq<double>(
                ...
                [&](const size_t vertex_id) {
                    ...
                    const auto get_intracluster_weight{[&](const uintE v_id,
                                                           const uintE ngh_id,
                                                           const Weight w) {
        ...
        const auto degrees_split_result{pbbs::split_two(
            pbbs::delayed_seq<std::pair<uintE, double>>(
                num_vertices,
                [&](const size_t i) {
        ...
        const auto &clusters_and_degrees{degrees_split_result.first};
        const auto &num_unclustered_vertices{degrees_split_result.second};
        // degrees_by_cluster[i] == sum of weighted degrees over vertices in
        // cluster
        // i
        const pbbs::sequence<double> degrees_by_cluster{internal::CollectReduce(
            clusters_and_degrees.slice(num_unclustered_vertices,
                                       clusters_and_degrees.size()),
            [&](const std::pair<uintE, double> p) { return p.first; },
            [&](const std::pair<uintE, double> p) { return p.second; },
            add_weights, num_clusters)};
        // Approximately the fraction edge weight that falls within a cluster
        // for a random graph with the same degree distribution.
        const double null_intracluster_proportion{
            pbbslib::reduce_add(pbbs::delayed_seq<double>(
                num_clusters,
                [&](const size_t cluster_id) {
                    return std::pow(
                        degrees_by_cluster[cluster_id] / total_weight, 2.0);
                })) +
            // special case for unclustered vertices
            pbbslib::reduce_add(pbbs::delayed_seq<double>(
                num_unclustered_vertices, [&](const size_t i) {
                    return std::pow(
                        clusters_and_degrees[i].second / total_weight, 2.0);
                }))};
