...
VertexSet MakeVertexSet(const size_t capacity) {
    return pbbslib::make_sparse_table<uintE, pbbslib::empty,
                                      decltype(&pbbslib::hash64_2)>(
        ...
        par_for(
            ...
            [&](const size_t j) {
                ...
                if (*neighbor_cluster_address == kUnclustered) {
                    pbbs::atomic_compare_and_swap(neighbor_cluster_address,
...
void AttachNoncoresToClustersDeterministic(
    ...
    // breaking ties in favor of the lowest vertex ID. (We break ties by vertex
    // ID instead of cluster ID so that we don't have to assume that cluster IDs
    // are deterministic.)

    // for a non-core vertex v, `tentative_attachments[v]` represents
    // (similarity score, vertex ID) of the most similar adjacent core to v.
...
Clustering Index::Cluster(const uint64_t mu, const float epsilon,
    ...
    if (cores.empty()) {
        // Nothing is a core. There are no clusters, and every vertex is an
        // outlier.
        return Clustering(num_vertices_, kUnclustered);
    }

    pbbs::sequence<uintE> core_similar_edge_counts{
        pbbs::map<uintE>(cores, [&](const uintE vertex) {
    ...
    if (get_deterministic_result) {
        ...
        AttachNoncoresToClusters(neighbor_order_, cores,
                                 core_similar_edge_counts, &clustering);
    }
    return clustering;
}

void Index::Cluster(const uint64_t mu, const pbbs::sequence<float> &epsilons,
    ...
    pbbs::sample_sort_inplace(sorted_epsilon_indices.slice(),
    ...
    for (size_t i{0}; i < epsilons.size(); i++) {
        ...
        // ClusterCores() logic -- get connected components induced by
        // sufficiently similar core-to-core edges
        ...
        par_for(0, cores.size(), [&](const size_t j) {
            ...
            // statement
            par_for(
                j < previous_cores.size() ? previous_core_similar_edge_counts[j]
                                          : 0,
                core_similar_edge_counts[j],
                [&](const size_t k) {
                    const uintE neighbor{neighbors[k].neighbor};
                    // the `core > neighbor` check tries to avoid the extra work
                    // of adding both directions of an edge. but sometimes when
                    // a new core is added we do need to check the other
                    // direction of an edge, hence the `j >=
