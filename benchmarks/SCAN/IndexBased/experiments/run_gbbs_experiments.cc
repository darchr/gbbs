...
template <class Graph> size_t GetMaxDegree(Graph *graph) {
    return pbbslib::reduce_max(
        pbbslib::make_sequence<size_t>(graph->n, [&](const size_t i) {
...
indexed_scan::Index
BuildIndexAndOutputTimes(Graph *graph,
...
void OutputQueryTimes(const indexed_scan::Index &index, const size_t max_degree,
    ...
    for (size_t mu{2}; mu <= max_degree + 1; mu *= 2) {
        ...
        for (size_t i{0}; i < num_rounds; i++) {
            timer query_timer{};
            // unused variable `clusters`, but keep it anyway so that it doesn't
            // get destructed within the timer
        ...
        std::cerr << "** Cluster median time "
                  << ParametersToString(mu, kEpsilon) << ": "
                  << Median(query_times) << "\n";
    }
    ...
    for (float epsilon : epsilons) {
        ...
        std::cerr << "** Cluster median time "
                  << ParametersToString(kMu, epsilon) << ": "
                  << Median(query_times) << "\n";
    }
    ...
    for (size_t i{0}; i < num_rounds; i++) {
        ...
        if (verbose) {
            std::cerr << ' ' << bulk_query_times.back();
        }
    }
    std::cerr << "** Bulk cluster median time mu=5: "
              << Median(bulk_query_times)
...
void OutputBuildApproximateIndexTimes(Graph *graph, const uint32_t num_samples,
    ...
    for (size_t i{0}; i < num_rounds; i++) {
        timer timer{"Index construction"};
        const indexed_scan::Index index{graph,
                                        SimilarityMeasure{num_samples, i}};
        times.emplace_back(timer.stop());
        if (verbose) {
            std::cerr << ' ' << times.back();
        }
...
void RunCosineExperiments(Graph *graph, const size_t num_index_rounds,
    ...
    std::cerr << "Best exact params "
              << ParametersToString(best_exact_clustering.mu,
