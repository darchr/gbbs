...
class Index {
  public:
    ...
    // Arguments:
    //   graph
    //     The graph on which to construct the index. The neighbor lists for
    //     each vertex in the graph must be sorted by ascending neighbor ID.
    ...
    //     vertices. The traditional choice for SCAN is
    //     `scan::CosineSimilarity`.
    ...
    explicit Index(
        ...
        : num_vertices_{graph->n}, neighbor_order_{graph, similarity_measure},
    ...
    //     on how much they share neighbors. Increasing this makes
    //     finer-grained, smaller clusters.
    //   mu
    //     How many neighbors a vertex needs to be epsilon-similar to in order
    ...
    //
    // Returns:
    //   `graph->n`-length sequence S where S[i] is the cluster ID of vertex i
    //   or is `kUnclustered` if vertex i does not belong to any cluster. The
    //   cluster IDs will be in the range `[0, graph->n)` but will not
    ...
    void Cluster(uint64_t mu, const pbbs::sequence<float> &epsilons,
