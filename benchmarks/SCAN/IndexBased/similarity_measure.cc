...
pbbs::sequence<float> RandomNormalNumbers(const size_t num_numbers,
                                          const pbbs::random rng) {
    using RngInt = std::result_of<decltype (&pbbs::random::ith_rand)(
        pbbs::random *, uint64_t)>::type;
