...
// vertices.
template <typename Weight> struct EdgeCounter {
    using type = int64_t; // weighted case
};

template <> struct EdgeCounter<pbbs::empty> {
...
pbbs::sequence<EdgeSimilarity> AllEdgeNeighborhoodSimilarities(
    ...
    // same as counting the number of triangles that the edge {u, v} appears in.
    //
    // The triangle counting logic here is borrowed from
    // `Triangle_degree_ordering()` in
    // `benchmarks/TriangleCounting/ShunTangwongsan15/Triangle.h`. We modify it
    // to maintain triangle counts for each edge.

    auto directed_graph{DirectGraphByDegree(graph)};
    // Each counter in `counters` holds the number of shared neighbors in
    // `graph` between u and v for some edge {u, v}.
    ...
    par_for(0, graph->n, [&](const size_t vertex_id) {
        ...
        const auto intersect{[&](const uintE v_id, const uintE neighbor_id,
                                 pbbs::empty, const uintE v_to_neighbor_index) {
...
pbbs::sequence<EdgeSimilarity> ApproxCosineEdgeSimilarities(
    ...
    // vectors by drawing a random n-dimensional hyperplane and determining
    // which side of the hyperplane the vectors fall on. The larger the angle
    // between the two vectors, the more likely that the two vectors will fall
    // on opposite sides of the hyperplane. Repeat this for several random
    // hyperplanes.
    // Represent a hyperplane by a vector orthogonal to that hyperplane.
    // Generate that uniformly random orthogonal vector by drawing i.i.d. normal
    // variables for each dimension. Determine which side of the hyperplane
    // vectors fall on by taking the dot product with the orthogonal vector.
    //
    // For edges between high degree vertices, estimate the similarity with
    // SimHash. For edges with a low degree vertex, compute the similarity
    // exactly with triangle counting like in
    ...
    // of the vector's dot product with each hyperplane. For efficiency, we
    // store the bits in chunks of `kBitArraySize` rather than one-by-one.
    ...
    par_for(0, graph->n, [&](const size_t vertex_id) {
        ...
        if (vertex.out_degree() >= degree_threshold) {
            // Vertex should be fingerprinted if both it and one of its
            // neighbors has high degree. If a vertex needs to be fingerprinted,
            // then normal random numbers should be generated for it and its
            // neighbors.
    ...
    pbbs::sequence<BitArray> fingerprints{pbbs::sequence<BitArray>::no_init(
    ...
    par_for(0, num_vertices, [&](const size_t vertex_id) {
        ...
        Vertex vertex{graph->get_vertex(vertex_id)};
        const uintE vertex_normal_offset{num_samples *
                                         normals_indices[vertex_id]};
        ...
        par_for(0, num_bit_arrays, [&](const size_t bit_array_id) {
            ...
            const size_t max_bit_id{
                bit_array_id + 1 == num_bit_arrays
                    ? kBitArraySize -
                          (num_bit_arrays * kBitArraySize - num_samples)
                    : kBitArraySize};
    ...
    //      /   \.
    //     u --> v
    // Count each of these triangles to get the number of shared neighbors
    // between vertices. However, we skip pairs of vertices that have high
    // degree in the original, undirected graph.
    par_for(0, graph->n, [&](const size_t vertex_id) {
        auto vertex{directed_graph.get_vertex(vertex_id)};
        ...
        if (vertex_is_high_degree) {
            ...
            // vertices, if the current vertex is high degree, so are all its
            // directed neighbors. Skip these pairs of edges since we'll
            // approximate their similarities.
            return;
        }
            ...
            if constexpr (std::is_same<Weight, pbbslib::empty>::value) {
                ...
                const auto update_counters{
                    [&](const uintE shared_neighbor,
                        ...
                        counters[vertex_counter_offset +
                                 vertex_to_shared_index]++;
                        if (!(neighbor_is_high_degree &&
                              graph->v_data[shared_neighbor].degree >=
                                  degree_threshold)) {
                            counters[neighbor_counter_offset +
                                     neighbor_to_shared_index]++;
                        }
                    }};
                counters[vertex_counter_offset + v_to_neighbor_index] +=
                ...
                const auto update_counters{
                    [&](const uintE shared_neighbor,
                        const uintE vertex_to_shared_index,
                        const uintE neighbor_to_shared_index,
                        const Weight weight_1, const Weight weight_2) {
                        counters[vertex_counter_offset +
                                 vertex_to_shared_index] +=
                        ...
                        if (!(neighbor_is_high_degree &&
                            ...
                            counters[neighbor_counter_offset +
                                     neighbor_to_shared_index] +=
                                kWeightFactor * kWeightFactor * weight *
                                weight_1;
                        }
                    }};
                counters[vertex_counter_offset + v_to_neighbor_index] +=
    ...
    const pbbs::sequence<double> norms{[&]() -> pbbs::sequence<double> {
        if constexpr (std::is_same<Weight,
                                   pbbslib::empty>::value) { // unweighted
            return {};
        } else { // weighted case
            return pbbs::sequence<double>{
                graph->n, [&](const size_t vertex_id) {
                    ...
                    constexpr bool kParallel{false};
                    const auto update_norm{[&](uintE, uintE,
                                               const Weight weight) {
    ...
    par_for(0, directed_graph.n, [&](const size_t vertex_id) {
        ...
        const auto compute_similarity{[&](const uintE v_id, const uintE u_id,
            ...
            if (vertex_is_high_degree) { // approximate similarity
                ...
                const auto fingerprint_xor{pbbs::delayed_seq<
                    std::remove_const<decltype(num_samples)>::type>(
                ...
                if constexpr (std::is_same<Weight, pbbslib::empty>::value) {
                    ...
                    // additional term is to account for self-loop edge in
                    // closed neighborhood
...
pbbs::sequence<EdgeSimilarity> ApproxJaccardEdgeSimilarities(
    ...
    // with a MinHash variant --- see paper "One Permutation Hashing for
    // Efficient Search and Learning." The MinHash variant works as follows to
    // estimate the Jaccard similarity between two sets using k samples:
    // partition the universe U of elements into k equally sized buckets U_1,
    // U_2, ..., U_k (e.g., permute U to assign each element x some value
    // permute(x) in [0, |U|), then assign x to bucket U_{(permute(x) % k) +
    // 1}). The fingerprint of a set S consists of (x_1,
    ...
    // distance between sets with fingerprints (x_1, ..., x_k) and (y_1, ...,
    // y_k) is <number of indices i where x_i == y_i != <empty>> / (k - <number
    // of indices where x_i == y_i == <empty>>).
    //
    // For edges with a low degree vertex, compute the Jaccard similarity
    // exactly with triangle counting like in
    // `AllEdgeNeighborhoodSimilarities()`.
    ...
    const pbbs::sequence<uintE> vertex_permutation{
        pbbs::random_permutation<uintE>(num_vertices,
                                        pbbs::random{random_seed})};
    ...
    pbbs::sequence<uintE> fingerprints(num_needs_fingerprint * num_samples,
                                       kEmptyBucket);
    par_for(0, num_vertices, [&](const size_t vertex_id) {
        const uintE fingerprint_index{fingerprint_indices[vertex_id]};
        const bool needs_fingerprint{
        ...
        const auto update_fingerprint{
            [&](uintE, const uintE neighbor, pbbs::empty) {
                ...
                pbbs::write_min(&(fingerprints[fingerprint_offset + bucket_id]),
    ...
    par_for(0, directed_graph.n, [&](const size_t vertex_id) {
        ...
        const auto compute_similarity{[&](const uintE v_id, const uintE u_id,
            ...
            if (vertex_is_high_degree) { // approximate similarity
                ...
                const uintE fingerprint_matches{pbbslib::reduce_add(
                    pbbs::delayed_seq<uintE>(num_samples, [&](const size_t i) {
                        return fingerprints[vertex_fingerprint_offset + i] !=
                                   kEmptyBucket &&
                               fingerprints[vertex_fingerprint_offset + i] ==
                                   fingerprints[neighbor_fingerprint_offset +
                                                i];
                    }))};
                const uintE fingerprint_empty_count{pbbslib::reduce_add(
                    pbbs::delayed_seq<uintE>(num_samples, [&](const size_t i) {
                        return fingerprints[vertex_fingerprint_offset + i] ==
                                   kEmptyBucket &&
                               fingerprints[neighbor_fingerprint_offset + i] ==
                ...
                similarity = static_cast<float>((num_shared_neighbors + 2)) /
                             neighborhood_union;
            }
            similarities[2 * counter_index] = {
                .source = v_id, .neighbor = u_id, .similarity = similarity};
...
pbbs::sequence<EdgeSimilarity> CosineSimilarity::AllEdges(
    ...
    if constexpr (std::is_same<Weight, pbbslib::empty>::value) { // unweighted
        constexpr auto similarity_func{[](const uintE neighborhood_size_1,
            ...
            // neighborhoods, hence the need to to adjust these values by `+ 1`
            // and
            // `+ 2`.
        ...
        return internal::AllEdgeNeighborhoodSimilarities(graph,
                                                         similarity_func);
    } else { // weighted case
        auto directed_graph{internal::DirectGraphByDegree(graph)};
        // Each counter in `counters` will hold numerator of CosineSimilarity(u,
        // v) for some edge {u, v}.
        pbbs::sequence<std::atomic<int64_t>> counters(
            directed_graph.m, [](size_t) { return std::atomic<int64_t>{0}; });
        // For floating-point-weighted graphs, we multiply all edge weights by
        // this factor and round to the nearest integer. Multiplying all edge
        // weights by a constant doesn't affect cosine similarities. The
        // multiplication preserves more accuracy after rounding.
        constexpr int64_t kWeightFactor =
            std::is_floating_point<Weight>::value ? 1000 : 1;
        // We use `counter_offsets` to index into `counters` for each edge.
        const pbbs::sequence<uintT> counter_offsets{
        ...
        //      /   \.
        //     u --> v
        // There's a bijection between triangles of this form in
        // `directed_graph` and undirected triangles in `graph`.
        par_for(0, graph->n, [&](const size_t vertex_id) {
            auto vertex{directed_graph.get_vertex(vertex_id)};
            const uintT vertex_counter_offset{counter_offsets[vertex_id]};
            const auto intersect{[&](const uintE v_id, const uintE neighbor_id,
                                     ...
                                     const uintE v_to_neighbor_index) {
                auto neighbor{directed_graph.get_vertex(neighbor_id)};
                const uintT neighbor_counter_offset{
                    counter_offsets[neighbor_id]};
                const auto update_counters{
                    [&](const uintE shared_neighbor,
                        const uintE vertex_to_shared_index,
                        const uintE neighbor_to_shared_index,
        ...
        par_for(0, directed_graph.n, [&](const size_t vertex_id) {
            const uintT v_counter_offset{counter_offsets[vertex_id]};
            const auto compute_similarity{[&](const uintE v_id,
                                              const uintE u_id,
                                              const Weight weight,
                                              const uintE v_to_u_index) {
                const uintT counter_index{v_counter_offset + v_to_u_index};
...
pbbs::sequence<EdgeSimilarity> JaccardSimilarity::AllEdges(
    ...
    constexpr auto similarity_func{[](const uintE neighborhood_size_1,
        ...
        const uintE neighborhood_union{
            neighborhood_size_1 + neighborhood_size_2 - num_shared_neighbors};
        // The `+ 2` accounts for the Jaccard similarity being computed with
        // respect to closed neighborhoods.
        return static_cast<float>((num_shared_neighbors + 2)) /
               neighborhood_union;
    }};
