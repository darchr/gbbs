...
struct vdata {
    ...
    __attribute__((always_inline)) inline uintE
    combine_root_rank(bool is_root, uintE _rank) const {
...
template <class S> void link(uintE u, uintE v, S &vdatas, pbbs::random r) {
    auto ud = vdatas[u];
    auto vd = vdatas[v];
...
template <class G, class Find> struct JayantiTBUnite {
    G &GA;
    Find &find;
            ...
            [&](size_t i) {
                ...
                if constexpr (provides_frequent_comp) {
                    if (parents[i] != frequent_comp) {
                        GA.get_vertex(i).out_neighbors().map(
                            map_f); // in parallel
                    }
                } else {
    ...
    void process_batch(pbbs::sequence<parent> &parents, Seq &batch) {
        ...
        // To enable correctness checking (otherwise the correct parent values
        // are stored in vdatas) parallel_for(0, n, [&] (size_t i) {
