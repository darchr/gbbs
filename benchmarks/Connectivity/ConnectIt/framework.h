...
template <FindOption find_option> auto get_find_function() {
...
template <SpliceOption splice_option> auto get_splice_function() {
...
/* Jayanti strategies */
template <JayantiFindOption find_option> auto get_jayanti_find_function() {
...
pbbs::sequence<parent> compose_algorithm_and_sampling(Graph &G, commandLine &P,
                                                      Algorithm &alg) {
    if constexpr (sampling_option == sample_kout) {
        ...
        auto connectivity =
            NoSamplingAlgorithmTemplate<Graph, Algorithm, algorithm_type>(G,
                                                                          alg);
        return connectivity.components();
    }
}
...
pbbs::sequence<parent> run_uf_alg(Graph &G, commandLine &P) {
    size_t n = G.n;
...
pbbs::sequence<parent> run_jayanti_alg(Graph &G, commandLine &P) {
    size_t n = G.n;
...
pbbs::sequence<parent> run_sample_only_alg(Graph &G, commandLine &P) {
...
pbbs::sequence<parent> run_liu_tarjan_alg(Graph &G, commandLine &P) {
...
template <
    ...
    LiuTarjanConnectOption connect_option, LiuTarjanUpdateOption update_option,
    LiuTarjanShortcutOption shortcut_option, LiuTarjanAlterOption alter_option>
pbbs::sequence<parent>
run_liu_tarjan_alg(Graph &G,
    ...
    using LT =
        lt::LiuTarjanAlgorithmCOO<decltype(alg_connect), connect_option,
                                  decltype(alg_update), update_option,
                                  decltype(alg_shortcut), shortcut_option,
                                  decltype(alg_alter), alter_option>;
