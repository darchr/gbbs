    ...
    pbbs::sequence<parent> components() {
        ...
        if constexpr (algorithm_type == liu_tarjan_type ||
            ...
            parallel_for(
                ...
                2048);
            algorithm.template compute_components<sampling_option>(
                parents, largest_comp);
            parallel_for(
                0, G.n,
                [&](size_t i) {
...
template <class G> struct KOutSamplingTemplate {
    G &GA;
    ...
    //  inline uintE my_split_atomic_one(uintE i, uintE x,
    ...
    void link(uintE u, uintE v, pbbs::sequence<parent> &parents) {
        ...
        //      }
        //      if (rx == parents[rx] &&
        //      pbbs::atomic_compare_and_swap(&parents[rx], rx, p_ry)) {
        //        break;
        //      } else {
        ...
        while (p1 != p2) {
            ...
            if ((p_high == low) ||
                (p_high == high &&
                 pbbs::atomic_compare_and_swap(&parents[high], high, low)))
                break;
    ...
    pbbs::sequence<parent> initial_components() {
        ...
        for (uint32_t r = 0; r < neighbor_rounds; r++) {
            if (r == 0) {
                /* First round: sample a directed forest and compress instead of
                 * using unite */
                parallel_for(
                    0, n,
                    [&](size_t u) {
                ...
                parallel_for(
                    ...
                    [&](size_t u) {
                        ...
                        if (u_vtx.out_degree() > 0) {
                            ...
                            auto reduce_m = pbbs::make_monoid(
                                reduce_f, std::make_pair(0, 0));
                            auto [ngh, degree] =
                                u_vtx.reduceOutNgh(u, map_f, reduce_m);
                            (void)degree;
                            link(u, ngh, parents);
                        }
                    },
    ...
    pbbs::sequence<parent> initial_components_pure() {
        ...
        pbbs::random rnd;
        uintE granularity = 1024;
        // Using random neighbor---some overhead (and faster for some graphs),
        // also theoretically defensible
        for (uint32_t r = 0; r < neighbor_rounds; r++) {
            parallel_for(
                0, n,
                [&](size_t u) {
...
template <class W> struct BFS_ComponentLabel_F {
    ...
    inline bool updateAtomic(const uintE &s, const uintE &d, const W &w) {
        return (pbbs::atomic_compare_and_swap(
            &Parents[d], static_cast<parent>(d), static_cast<parent>(src)));
    }
...
template <class G> struct BFSSamplingTemplate {
    G &GA;
    ...
    pbbs::sequence<parent> initial_components() {
        ...
        for (uint32_t r = 0; r < max_trials; r++) {
            ...
            if (pct > static_cast<double>(0.1)) {
                std::cout << "# BFS covered: " << pct << " of graph"
                          << std::endl;
                break;
            }
            std::cout << "# BFS covered only: " << pct << " of graph."
                      << std::endl;
            rnd = rnd.next();
        }
...
template <class G> struct LDDSamplingTemplate {
    G &GA;
    double beta;
    bool permute;
