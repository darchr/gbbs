...
template <class Graph> double TestEdgeGather(Graph &G, commandLine &P) {
...
template <class Graph> double TestEdgeMap(Graph &G, commandLine &P) {
    using W = typename Graph::weight_type;
    long rounds = P.getOptionLongValue("-r", 5);
    size_t n = G.n;
    auto test = [&]() {
...
template <class Graph> double Sampler(Graph &G, commandLine &P) {
    std::cout << "[" << std::endl;
