...
template <class Stats> void print_cpu_stats(Stats &stats, commandLine &P) {
    std::cout
        << "Stats = { \"ipc\":" + std::to_string(stats.get_ipc()) +
               " ,\"total_cycles\":" +
               std::to_string(stats.get_total_cycles()) +
               " ,\"l2_hit_ratio\":" +
               std::to_string(stats.get_l2_hit_ratio()) +
               " ,\"l3_hit_ratio\":" +
               std::to_string(stats.get_l3_hit_ratio()) +
               ...
               " ,\"total_path_len\":" +
               std::to_string(total_pathlen.get_value()) +
               " ,\"max_uf_tries\":" +
               std::to_string(max_uf_tries.get_value()) +
               " ,\"total_uf_tries\":" +
               std::to_string(total_uf_tries.get_value()) + "}"
        << std::endl;
}
} // namespace gbbs
