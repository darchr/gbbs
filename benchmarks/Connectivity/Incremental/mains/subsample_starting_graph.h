...
template <class Graph> double Run(Graph &G, commandLine P) {
    ...
    auto hash_to_double = [&](const uintE &u, const uintE &v) -> double {
        ...
        size_t hashed_v = pbbs::hash64((static_cast<size_t>(min_v) << 32UL) +
    ...
    tt.stop();
    tt.reportTotal("# sample edges time");
    auto updates = pbbs::sequence<std::tuple<uintE, uintE>>(
        (std::tuple<uintE, uintE> *)updates_arr.E, updates_arr.m);
    ...
    //    insert_to_query, n, permute); auto FG =
    //    gbbs::edge_array<pbbs::empty>(); run_all_tests<decltype(FG),
    //    false>(FG, n, annotated_updates, batch_size, insert_to_query, rounds,
    //    P);
    //}
