...
template <typename F> double reduce(std::vector<double> V, F f) {
    double x = V[0];
...
void print_cpu_stats(std::string &name, size_t rounds, size_t cc_before,
                     size_t cc_after, double medt, double mint, double maxt,
                     double med_batch, double min_batch, double max_batch,
                     double med_throughput, double min_throughput,
                     double max_throughput, CPUStats &stats, commandLine &P) {
    std::cout << "{" << std::endl;
    std::cout << "  \"test_type\": \"static_connectivity_result\","
              << std::endl;
    ...
    std::cout << "  \"med_throughput\" : " << med_throughput << ","
              << std::endl;
    std::cout << "  \"min_throughput\" : " << min_throughput << ","
              << std::endl;
    std::cout << "  \"max_throughput\" : " << max_throughput << ","
              << std::endl;
    std::cout << "  \"ipc\" : " << std::to_string(stats.get_ipc()) << ","
              << std::endl;
    std::cout << "  \"total_cycles\" : "
              << std::to_string(stats.get_total_cycles()) << "," << std::endl;
    std::cout << "  \"l2_hit_ratio\" : "
              << std::to_string(stats.get_l2_hit_ratio()) << "," << std::endl;
    std::cout << "  \"l3_hit_ratio\" : "
              << std::to_string(stats.get_l3_hit_ratio()) << "," << std::endl;
    std::cout << "  \"l2_misses\" : " << std::to_string(stats.get_l2_misses())
              << "," << std::endl;
    std::cout << "  \"l2_hits\" : " << std::to_string(stats.get_l2_hits())
              << "," << std::endl;
    std::cout << "  \"l3_misses\" : " << std::to_string(stats.get_l3_misses())
              << "," << std::endl;
    std::cout << "  \"l3_hits\" : " << std::to_string(stats.get_l3_hits())
              << "," << std::endl;
    std::cout << "  \"throughput\" : " << std::to_string(stats.get_throughput())
              << "," << std::endl;
...
bool run_multiple(Graph &G, size_t rounds, std::string name, commandLine &P,
    ...
    print_cpu_stats(name, rounds, cc_before, cc_after, medt, mint, maxt, meda,
                    mina, maxa, medtp, mintp, maxtp, stats, P);
    return 1;
}

template <class Graph, bool provides_initial_graph, class F>
void run_tests(Graph &G, size_t n, pbbs::sequence<incremental_update> &updates,
 ...
 */
template <class S1, class S2> inline void cc_check(S1 &correct, S2 &check);
...
template <class Graph, class Alg, bool provides_initial_graph,
          bool reorder_batch>
auto run_abstract_alg(Graph &G, size_t n,
    ...
    if constexpr (provides_initial_graph) {
        ...
        auto unite_fn =
            get_unite_function<unite, decltype(find_fn), find_compress>(
                n, find_fn);
        using UF = union_find::UFAlgorithm<decltype(find_fn),
                                           decltype(unite_fn), Graph>;
    ...
    init_t.stop();
    init_t.reportTotal("#alg initialization time");
    ...
    return std::make_tuple(t, med_batch_time, throughput, ncc_before,
                           ncc_after);
}
