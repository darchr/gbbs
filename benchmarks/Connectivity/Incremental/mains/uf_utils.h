...
bool run_multiple_uf_alg(Graph &G, size_t n,
    ...
    auto test = [&](Graph &graph, commandLine params) {
        ...
        /* NOTE: none of the algorithms going through this step require
         * reordering a batch */
...
bool run_multiple_uf_alg(Graph &G, size_t n,
    ...
    auto test = [&](Graph &graph, commandLine params) {
        ...
        using UF =
            union_find::UFAlgorithm<decltype(find), decltype(unite), Graph>;
        auto alg = UF(graph, unite, find);

        bool check = params.getOptionValue("-check");
        if constexpr (splice_option == splice_atomic) {
            return run_abstract_alg<Graph, decltype(alg),
