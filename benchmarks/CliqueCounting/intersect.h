...
struct HybridSpace_lw {
    ...
    // recursive level
    uintE num_edges = 0;
    // Edges in induced subgraph on induced neighbors, from first recursive
    // level
    uintE *induced_edges = nullptr;
    // Degrees of each induced neighbor in induced subgrpah, from first
    // recursive level
    uintE *induced_degs = nullptr;
    // Label each induced neighbor with latest recursive level it was included
    // in
    char *labels = nullptr;
    ...
    void alloc(size_t max_induced, size_t k, size_t n, bool _use_old_labels,
        ...
        if (labels == nullptr)
            labels = (char *)calloc(max_induced, sizeof(char));
        if (free_relabel && induced_edges == nullptr)
            induced_edges =
                (uintE *)malloc(sizeof(uintE) * max_induced * max_induced);
        if (num_induced == nullptr)
    ...
    template <class Graph> void setup(Graph &DG, size_t k, size_t i) {
    ...
    void setup_intersect(Graph &DG, Graph2 &DG2, size_t k, size_t i, F f) {
        ...
        if (use_base) {
            size_t j = 0;
            auto map_base_f = [&](const uintE &src, const uintE &v,
                                  const W &wgh) {
                relabel[j] = v;
                j++;
            };
        ...
        // Set up first level induced neighborhood (neighbors of vertex i,
        // relabeled from 0 to degree of i)
        ...
        auto map_f = [&](const uintE &src, const uintE &v, const W &wgh) {
            ...
            // These are the edges adjacent to v in the induced neighborhood of
            // i Note that v is relabeled to j under the relabeling from 0 to
            // degree of i Store these edges in induced_edges[j*nn] Store the
            // number of edge (degree) in induced_degs[j]
            ...
            while (i_iter_idx < nn && v_iter_idx < v_deg) {
                ...
                if (std::get<0>(i_iter.cur()) == std::get<0>(v_iter.cur())) {
                    // Check if the edges (i, i_iter.cur()) and (v,
                    // v_iter.cur()) are valid
                    if (f(i, std::get<0>(i_iter.cur())) &&
                        f(v, std::get<0>(i_iter.cur()))) {
                        // Save the neighbor and increment the induced degree on
                        // j
                    ...
                    if (v_iter.has_next())
                        v_iter.next();
                } else if (std::get<0>(i_iter.cur()) <
                           std::get<0>(v_iter.cur())) {
                    i_iter_idx++;
                    if (i_iter.has_next())
                        i_iter.next();
                } else {
    ...
    void setup_labels(Graph &DG, Graph2 &DG2, size_t k, size_t i, F f) {
        ...
        size_t o = 0;
        auto map_label_f = [&](const uintE &src, const uintE &ngh,
                               const W &wgh) {
        ...
        auto map_f = [&](const uintE &src, const uintE &v, const W &wgh) {
            ...
            auto map_nbhrs_f = [&](const uintE &src_v, const uintE &v_nbhr,
                ...
                if (old_labels[v_nbhr] > 0) {
                    ...
                    induced_edges[j * nn + induced_degs[j]] =
                        old_labels[v_nbhr] - 1;
                    induced_degs[j]++;
                }
            };
        ...
        auto map_relabel_f = [&](const uintE &src, const uintE &ngh,
    ...
    void setup_labels_edge(Graph &DG, size_t k, size_t i, size_t l, F f) {
        ...
        size_t o = 0;
        auto lmap_label_f = [&](const uintE &src, const uintE &ngh,
                                const W &wgh) {
            // Return if edge is invalid
            if (!f(src, ngh)) {
                return;
        ...
        // Reset induced neighbors from the first recursive level but not the
        // second
        auto remap_label_f = [&](const uintE &src, const uintE &ngh,
                                 const W &wgh) {
            if (old_labels[ngh] == UINT_E_MAX)
                old_labels[ngh] = 0;
        };
        ...
        for (size_t j = 0; j < nn; j++) {
            auto v = induced[j];
            // For a neighbor v in the second level induced neighborhood,
            // intersect N(v) with the second level induced neighborhood Note
            // that v is relabeled to j under the relabeling Store these edges
            // in induced_edges[j*nn] Store the number of edge (degree) in
            // induced_degs[j]
            auto map_nbhrs_f = [&](const uintE &src_v, const uintE &v_nbhr,
                                   const W &wgh_v) {
    ...
    void setup_intersect_edge(Graph &DG, size_t k, size_t i, size_t l, F f) {
        ...
        for (size_t p = 0; p < nn; p++) {
            // For a neighbor v = induced[p] in the second level induced
            // neighborhood, intersect N(v) with the second level induced
            // neighborhood Note that v is relabeled to p under the relabeling
...
struct InducedSpace_lw {
    ...
    uintE *num_induced = nullptr;
    // Array storing induced neighbor list, for all levels of recursion. Levels
    // go from i=0 ... k-1. For each i, the induced neighbor list at level i is
    // induced + num_induced[0]*i (num_induced[0] is an upper-bound on the
    // maximum number of induced neighbors at any level).
    uintE *induced = nullptr;
