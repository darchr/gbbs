...
inline symmetric_graph<csv_byte, W>
relabel_graph(symmetric_graph<vertex, W> &GA, uintE *rank,
              P &pred) { // -> decltype(GA)
    size_t n = GA.n;
    ...
    parallel_for(
        ...
        [&](size_t i) {
            ...
            // here write out all of G's outneighbors to an uncompressed array,
            // and then relabel and sort
            ...
            // now need to compute total_bytes (TODO: this could be
            // parallelized, b/c we know what last_ngh is)
    ...
    parallel_for(
        ...
        [&](size_t i) {
            uintE deg = degrees[i];
            if (deg > 0) {
                // here write out all of G's outneighbors to an uncompressed
                // array, and then relabel and sort
                ...
                byte::sequentialCompressEdgeSet<W>(edges.begin() +
                                                       byte_offsets[rank[i]],
                                                   0, deg, rank[i], iter);
            }
        },
    ...
    return symmetric_graph<csv_byte, W>(
        out_vdata, GA.n, total_deg,
        [=]() { pbbslib::free_arrays(out_vdata, edge_arr); }, edge_arr,
        edge_arr);
}
...
relabel_graph(symmetric_graph<vertex, W> &GA, uintE *rank, P &pred) {
        ...
        [&](size_t i) {
            ...
            if (d > 0) {
                ...
                parallel_for(0, true_deg, [&](size_t j) {
                    dir_nghs[j] =
                        std::make_tuple(rank[std::get<0>(dir_nghs[j])],
...
auto clr_sparsify_graph(Graph &GA, size_t denom, long seed) {
    ...
    sequence<uintE> colors = sequence<uintE>(n, [&](size_t i) {
        return pbbs::hash64_2((uintE)seed + i) % numColors;
    });
