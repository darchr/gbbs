...
inline size_t KCliqueDir_fast_rec(Graph &DG, size_t k_idx, size_t k,
    ...
    if (k_idx + 1 == k) {
        ...
        for (size_t i = 0; i < num_induced; i++) {
            ...
            auto out_f = [&](size_t j, const std::tuple<uintE, W> &nw) {
                tmp_counts++;
                if (use_base)
                    base_f(std::get<0>(nw), 1);
            };
            DG.get_vertex(vtx).out_neighbors().filter(pred, out_f, tmp.begin());
            if (use_base && tmp_counts > 0)
                base_f(vtx, tmp_counts);
            counts += tmp_counts;
            tmp_counts = 0;

            /*auto map_intersect_f = [&] (const uintE& src, const uintE& nbhr,
            const W& wgh) { if (induced->intersect[nbhr] == k_idx) counts++;
            };
    ...
    for (size_t i = 0; i < num_induced; ++i) {
        ...
        // induced->num_induced[k_idx] = lstintersect_set(prev_induced,
        // num_induced, (uintE*)(DG.get_vertex(vtx).getOutNeighbors()),
        // DG.get_vertex(vtx).out_degree(), true, induced->induced +
        // induced->num_induced[0] * k_idx);
        if (induced->num_induced[k_idx] > 0) {
            auto curr_ct = KCliqueDir_fast_rec(DG, k_idx + 1, k, induced,
                                               base_f, use_base);
            if (use_base && curr_ct > 0)
                base_f(vtx, curr_ct);
            total_ct += curr_ct;
        }
    }
    ...
    parallel_for_alloc<InducedSpace_lw>(
        ...
        [&](size_t i, InducedSpace_lw *induced) {
            if (DG.get_vertex(i).out_degree() != 0) {
                ...
                tots[i] =
                    KCliqueDir_fast_rec(DG, 1, k, induced, base_f, use_base);
                if (use_base && tots[i] > 0)
                    base_f(i, tots[i]);
            } else
                tots[i] = 0;
        });
