...
template <class Graph> size_t get_max_deg(Graph &DG) {
    size_t max_deg = 0;
    ...
    if (k_idx + 1 == k) {
        ...
        for (size_t i = 0; i < num_induced; i++) {
            ...
            for (size_t j = 0; j < induced->induced_degs[vtx]; j++) {
                if (static_cast<size_t>(induced->labels[intersect[j]]) ==
                    k_idx) {
                    tmp_counts++;
                    if (induced->use_base)
                        base_f(induced->relabel[intersect[j]], 1);
                }
            }
    ...
    if (recursive_level < k_idx || num_induced < 2) {
        for (size_t i = 0; i < num_induced; ++i) {
            ...
            if (induced->num_induced[k_idx] > k - k_idx - 1) {
                auto curr_counts = KCliqueDir_fast_hybrid_rec(
                    DG, k_idx + 1, k, induced, base_f, recursive_level);
        ...
        auto init_induced = [&](HybridSpace_lw *induced2) {
            induced2->alloc(induced->nn, k - k_idx, DG.n, false,
                            induced->use_base, false);
        };
        ...
        parallel_for_alloc<HybridSpace_lw>(
            ...
            [&](size_t i, HybridSpace_lw *induced2) {
                ...
                if (count > k - k_idx - 1) {
                    tots[i] = KCliqueDir_fast_hybrid_rec(DG, 1, k - k_idx,
                                                         induced2, base_f);
...
inline size_t CountCliques(Graph &DG, size_t k, F base_f, bool use_base = false,
        ...
        parallel_for_alloc<HybridSpace_lw>(
            ...
            [&](size_t i, HybridSpace_lw *induced) {
                if (DG.get_vertex(i).out_degree() != 0) {
                    ...
                    tots[i] = KCliqueDir_fast_hybrid_rec(
    ...
    parallel_for_alloc<HybridSpace_lw>(
        ...
        [&](size_t j, HybridSpace_lw *induced) {
            // to find i and ngh, binary search for j in degs; the index - 1 is
            // i then, DG.get_vertex(i).getOutNeighbor(j - degs[index-1]) is ngh
            auto less_fn = [&](size_t a, size_t b) { return a <= b; };
            size_t idx = pbbslib::binary_search(degs, j, less_fn);
            auto i = idx - 1;
            auto ngh = DG.get_vertex(i).out_neighbors().get_neighbor(
                j - degs[idx - 1]);
            induced->setup_edge(DG, k, i, ngh);
            tots[j] =
                KCliqueDir_fast_hybrid_rec(DG, 1, k - 1, induced, base_f, 0);
