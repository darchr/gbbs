    ...
    while (i_iter_idx < i_deg && v_iter_idx < v_deg) {
        if (std::get<0>(i_iter.cur()) == std::get<0>(v_iter.cur())) {
            ...
            if (ignore_f(vg, nbhr) && ignore_f(vdg, nbhr)) {
                // If the neighbor is valid (ignore_f), one triangle is added to
                // the neighbor
                f(nbhr, 1);
                j++;
            }
...
inline size_t
triUpdate_serial(Graph &G, Graph2 &DG, F get_active, size_t active_size,
                 char *still_active, sequence<uintE> &rank,
                 sequence<size_t> &per_processor_counts, bool do_update_changed,
                 I update_changed, sequence<uintE> &update_idxs) {
    using W = typename Graph::weight_type;

    // Mark every vertex in the active set
    parallel_for(
        0, active_size, [&](size_t j) { still_active[get_active(j)] = 1; },
        2048);

    size_t num_updates = 0;

    // Function that dictates which edges to consider in first level of
    // recursion
    auto ignore_f = [&](const uintE &u, const uintE &v) {
        auto status_u = still_active[u];
        auto status_v = still_active[v];
        if (status_u == 2 || status_v == 2)
            return false; // deleted edge
        if (status_v == 0)
            return true;          // non-deleted, non-active edge
    ...
    parallel_for(
        0, active_size, [&](size_t j) { still_active[get_active(j)] = 2; },
        2048);
...
inline size_t triUpdate(Graph &G, Graph2 &DG, F get_active, size_t active_size,
    ...
    size_t active_deg = 0;
    auto degree_map =
        pbbslib::make_sequence<size_t>(active_size, [&](size_t i) {
    ...
    auto edge_table = pbbslib::sparse_table<uintE, bool, hashtup>(
    ...
    parallel_for(
        ...
        [&](size_t i) {
            auto j = get_active(i);
            // For each neighbor v of active vertex j = u, intersect N(v) with
            // N(j)
    ...
    // Aggregate the updated counts across all worker's local arrays, as
    // specified by update
    parallel_for(
        0, changed_vtxs.size(),
        [&](size_t i) {
    ...
    if (do_update_changed) {
        parallel_for(0, changed_vtxs.size(), [&](size_t i) {
            update_changed(per_processor_counts, i,
                           std::get<0>(changed_vtxs[i]));
        });
    }
...
sequence<bucket_t> Peel(Graph &G, Graph2 &DG, size_t k, size_t *cliques,
    ...
    auto b =
        make_vertex_custom_buckets<bucket_t>(n, D, increasing, num_buckets);
    ...
    while (finished != n) {
        ...
        if (active.size() > 1) {
            auto update_changed = [&](sequence<size_t> &ppc, size_t i,
                                      uintE v) {
                /* Update the clique count for v, and zero out first worker's
                 * count */
                cliques[v] -= ppc[v];
                ppc[v] = 0;
                bucket_t deg = D[v];
                if (deg > cur_bkt) {
                    bucket_t new_deg =
                        std::max((bucket_t)cliques[v], (bucket_t)cur_bkt);
                    D[v] = new_deg;
                    // store (v, bkt) in an array now, pass it to apply_f below
                    // instead of what's there right now -- maybe just store it
                    // in D_filter?
                    D_filter[i] =
            ...
            if (k == 2)
                filter_size =
                    triUpdate(G, DG, get_active, active.size(), granularity,
                              still_active, rank, per_processor_counts,
                              update_clique, true, update_changed);
            else
                filter_size = cliqueUpdate(
                    G, DG, k, max_deg, label, get_active, active.size(),
            ...
            if (k == 2)
                triUpdate_serial(G, DG, get_active, active.size(), still_active,
                                 rank, per_processor_counts, true,
                                 update_changed_serial, update_idxs);
            else
                cliqueUpdate_serial(G, DG, k, max_deg, label, get_active,
                                    active.size(), still_active, rank,
                                    per_processor_counts, true,
                                    update_changed_serial, update_idxs);
            filter_size = filter_size_serial;
        }

        auto apply_f =
            [&](size_t i) -> std::optional<std::tuple<uintE, bucket_t>> {
            uintE v = std::get<0>(D_filter[i]);
            bucket_t bucket = std::get<1>(D_filter[i]);
            if (v != UINT_E_MAX && still_active[v] != 2)
                return wrap(v, bucket);
            return std::nullopt;
        };
        ...
        parallel_for(
            0, active.size(), [&](size_t j) { cliques[active.vtx(j)] = 0; },
            2048);
...
double ApproxPeel(Graph &G, Graph2 &DG, size_t k, size_t *cliques,
    ...
    // First round
    {
        ...
        double target_density =
            (density_multiplier * ((double)edges_remaining)) /
            ((double)vertices_remaining.size());
        auto rho = target_density;
        if (current_density > max_density)
            max_density = current_density;

        auto keep_seq = pbbs::delayed_seq<bool>(
        ...
        parallel_for(
            0, active_size, [&](size_t j) { D[this_arr[j]] = 0; }, 2048);
        //***************
    ...
    while (num_vertices_remaining > 0) {
        ...
        auto degree_seq =
            pbbslib::make_sequence<size_t>(vtxs_remaining.size(), degree_f);
        ...
        auto keep_seq =
            pbbs::delayed_seq<bool>(vtxs_remaining.size(), [&](size_t i) {
        ...
        if (num_vertices_remaining > 0) {
            ...
            auto get_active = [&](size_t j) { return this_arr[j]; };
            if (k == 2)
                triUpdate(G, DG, get_active, active_size, granularity,
                          still_active, rank, per_processor_counts,
                          update_clique, false, nop);
            else
                cliqueUpdate(G, DG, k, max_deg, label, get_active, active_size,
                             granularity, still_active, rank,
    ...
    std::cout << "### Density of (2(1+\eps))-Densest Subgraph is: "
