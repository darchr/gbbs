...
inline size_t KCliqueDir_fast_orig_rec(Graph &DG, size_t k_idx, size_t k,
    ...
    uintE *prev_induced_degs =
        induced->induced_degs + induced->nn * (k_idx - 1);
    if (num_induced == 0)
        return 0;

    if (k_idx + 1 == k)
        return induced->num_edges[k_idx - 1];
    ...
    for (size_t i = 0; i < num_induced; ++i) {
        ...
        // parallel_for(0, induced->nn, [&] (size_t j) { new_induced_degs[j] =
        // 0;
        // });
...
template <class Graph> inline size_t CountCliques(Graph &DG, size_t k) {
