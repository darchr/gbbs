...
bool run_multiple_uf_alg(Graph &G, size_t rounds, pbbs::sequence<edge> &correct,
                         commandLine &P) {
...
bool run_multiple_uf_alg(Graph &G, size_t rounds, pbbs::sequence<edge> &correct,
    ...
    auto test = [&](Graph &G, commandLine P, pbbs::sequence<edge> &correct) {
        timer tt;
        tt.start();
        auto edges = run_uf_alg<Graph, sampling_option, find_option,
                                unite_option, splice_option>(G, P);
        double t = tt.stop();
        if (P.getOptionValue("-check")) {
            spanning_forest::check_spanning_forest(G.n, correct, edges);
        }
...
bool run_multiple_jayanti_alg(Graph &G, size_t rounds,
                              pbbs::sequence<edge> &correct, commandLine &P) {
    auto test = [&](Graph &G, commandLine P, pbbs::sequence<edge> &correct) {
        timer tt;
        tt.start();
