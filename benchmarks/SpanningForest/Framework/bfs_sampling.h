...
template <class W> struct BFS_ComponentLabel_F {
    ...
    uintE src;
    BFS_ComponentLabel_F(pbbs::sequence<parent> &_Parents, uintE src)
    ...
    inline bool updateAtomic(const uintE &s, const uintE &d, const W &w) {
        if (Parents[d] != src &&
            pbbs::atomic_compare_and_swap(&Parents[d], static_cast<parent>(d),
...
template <class Graph> inline auto BFS_ComponentLabel(Graph &G, uintE src) {
    ...
    auto initial_spanning_forest() {
        ...
        for (uint32_t r = 0; r < max_trials; r++) {
            ...
            if (pct > static_cast<double>(0.1)) {
                std::cout << "# BFS from " << src << " covered: " << pct
                          << " of graph" << std::endl;
                break;
            }
