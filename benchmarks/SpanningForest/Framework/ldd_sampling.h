...
namespace gbbs {
template <class W> struct LDD_Edges_Fn {
    ...
    inline bool updateAtomic(const uintE &s, const uintE &d, const W &wgh) {
        if (pbbslib::atomic_compare_and_swap(&Parents[d], UINT_E_MAX,
                                             Parents[s])) {
            Edges[d] = std::make_pair(s, d);
            return true;
        }
...
inline std::pair<pbbs::sequence<uintE>, pbbs::sequence<edge>>
LDD_sample_edges(Graph &G, double beta, bool permute = true,
                 bool pack = false) {
...
template <class G> struct LDDSamplingTemplate {
    ...
    auto initial_spanning_forest() {
        ...
        lddt.start();
        auto [Parents, Edges] =
            LDD_sample_edges(GA, 0.2, /* permute = */ false);
        lddt.stop();
