...
inline uintE split_atomic_one(uintE i, uintE x,
...
/* Used in Rem-CAS variants for splice */
inline uintE halve_atomic_one(uintE i, uintE x,
                              pbbs::sequence<parent> &Parents) {
    parent v = Parents[i];
    parent w = Parents[v];
    if (v == w)
...
template <class Find> struct Unite {
    Find &find;
    Unite(Find &find) : find(find) {}

    inline void operator()(uintE u_orig, uintE v_orig,
                           pbbs::sequence<parent> &Parents,
        ...
        while (1) {
            ...
            if (u == v)
                break;
                     ...
                     pbbs::atomic_compare_and_swap(&Parents[u], u, v)) {
                Edges[u] = std::make_pair(u_orig, v_orig);
                break;
            } else if (v > u && Parents[v] == v &&
                       pbbs::atomic_compare_and_swap(&Parents[v], v, u)) {
...
template <class Compress, class Splice> struct UniteRemLock {
    uintE n;
    std::mutex *locks;
    Compress &compress;
    Splice &splice;
    ...
    ~UniteRemLock() { pbbs::free_array(locks); }
...
template <class Find> struct UniteND {
    Find find;
    ...
    inline void operator()(uintE u_orig, uintE v_orig,
        ...
        while (1) {
            ...
            if (hooks[u] == UINT_E_MAX &&
                pbbs::atomic_compare_and_swap(&hooks[u], UINT_E_MAX, v)) {
                Edges[u] = std::make_pair(u_orig, v_orig);
                Parents[u] = v;
                break;
            }
