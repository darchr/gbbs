...
void link(uintE u, uintE v, S &vdatas, pbbs::random r,
          pbbs::sequence<edge> &Edges, edge orig_edge) {
    auto ud = vdatas[u];
    auto vd = vdatas[v];
    ...
    if (ud.get_rank() < vd.get_rank()) { // u.r < v.r
        ...
        if (pbbs::atomic_compare_and_swap(&(vdatas[u]), expected_u, new_u)) {
            Edges[u] = orig_edge;
        }
        ...
        if (pbbs::atomic_compare_and_swap(&(vdatas[v]), expected_v, new_v)) {
            Edges[v] = orig_edge;
        }
    } else { // u.r == v.r
        ...
        if (u < v) {
            ...
            if (pbbs::atomic_compare_and_swap(&(vdatas[u]), expected_u,
                                              new_u) &&
                random_bit == 0) {
                Edges[u] = orig_edge;
            }
            ...
            if (pbbs::atomic_compare_and_swap(&(vdatas[v]), expected_v,
                                              new_v) &&
                random_bit == 0) {
                Edges[v] = orig_edge;
            }
