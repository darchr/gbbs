...
// of the received vertices to the maximum of the received vertices.
bool parent_connect(uintE u, uintE v, pbbs::sequence<parent> &P,
    ...
    if (min_v != max_v) {
        return pbbs::write_min<message>(&messages[max_v],
                                        message(min_v, std::make_pair(u, v)),
                                        message_less);
    }
    return false;
}
...
bool extended_connect(uintE v, uintE w, pbbs::sequence<parent> &P,
    ...
    if (lt_less(y, x)) { /* send y to {v, x}*/
        updated |= pbbs::write_min(
            &messages[v], message(y, std::make_pair(v, w)), message_less);
        updated |= pbbs::write_min(
            &messages[x], message(y, std::make_pair(v, w)), message_less);
    } else if (lt_greater(y, x)) { /* send x to {y, w} */
        updated |= pbbs::write_min(
            &messages[y], message(x, std::make_pair(v, w)), message_less);
        updated |= pbbs::write_min(
            &messages[w], message(x, std::make_pair(v, w)), message_less);
    }
    return updated;
}
...
std::tuple<uintE, uintE> alter(uintE u, uintE v, pbbs::sequence<parent> &P) {
    uintE x = P[u];
    uintE y = P[v];
