...
template <class Seq> inline size_t num_cc(Seq &labels) {
...
template <class Seq> inline void RelabelDet(Seq &ids) {
...
pbbs::sequence<std::tuple<uintE, uintE, pbbs::empty>>
double_edges(pbbs::sequence<edge> &in) {
    using weighted_edge = std::tuple<uintE, uintE, pbbs::empty>;
    auto double_in =
        pbbs::sequence<weighted_edge>(in.size() * 2, [&](size_t i) {
            size_t ind = i / 2;
            auto [u, v] = in[ind];
            if (i % 2 == 0) {
...
void check_spanning_forest(size_t n, pbbs::sequence<edge> &correct,
                           pbbs::sequence<edge> &check) {
    // check sizes
