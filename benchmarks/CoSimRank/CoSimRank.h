...
T inner_product(T *arr1, T *arr2, size_t len, T *tmp = nullptr) {
    ...
    if (tmp == nullptr)
        prod = pbbs::sequence<T>(len, static_cast<T>(0));
    else
        prod = pbbs::sequence<T>(tmp, len);
    ...
    while (iter++ < max_iters) {
        ...
        auto Frontier_v_new =
            edgeMap(G, Frontier_v,
                    PR_F<Graph>(p_curr_v.begin(), p_next_v.begin(), G), 0);
        auto Frontier_u_new = edgeMap(
            G, Frontier_u, PR_F<Graph>(p_curr_u.begin(), p_next_u.begin(), G),
            0); //, no_output

        sim += ((double)pow(c, iter) *
                inner_product<double>(p_next_u.begin(), p_next_v.begin(), n));
        ...
        // Reset p_curr
        parallel_for(0, n,
                     [&](size_t i) { p_curr_v[i] = static_cast<double>(0); });
        std::swap(p_curr_v, p_next_v);

        parallel_for(0, n,
                     [&](size_t i) { p_curr_u[i] = static_cast<double>(0); });
