...
inline vertexSubsetData<data> edgeMapSparse(Graph &G, VS &indices, F &f,
    ...
    parallel_for(
        ...
        [&](size_t i) {
            uintT v = indices.vtx(i);
            auto neighbors = (fl & in_edges) ? G.get_vertex(v).in_neighbors()
                                             : G.get_vertex(v).out_neighbors();
...
inline vertexSubsetData<data> edgeMapBlocked(Graph &G, VS &indices, F &f,
    ...
    auto block_f = [&](size_t i) -> size_t {
        return (fl & in_edges) ? G.get_vertex(indices.vtx(i))
                                     .in_neighbors()
                                     .get_num_blocks()
                               : G.get_vertex(indices.vtx(i))
                                     .out_neighbors()
                                     .get_num_blocks();
    };
    ...
    parallel_for(
        ...
        [&](size_t i) {
            ...
            parallel_for(0, num_vertex_blocks, [&](size_t j) {
                ...
                blocks[vtx_off + j] =
                    block(i, j); // j-th block of the i-th vertex.
...
template <class data, class Graph> struct emhelper {
    ...
    static constexpr size_t kThreadBlockStride =
        1; // 128/sizeof(thread_blocks);

    emhelper(size_t n_groups) : n_groups(n_groups) {
        perthread_blocks =
            pbbs::new_array<thread_blocks>(n_groups * kThreadBlockStride);
...
template <class Graph> void alloc_init(Graph &G) {
...
inline vertexSubsetData<data> edgeMapChunked(Graph &G, VS &indices, F &f,
    ...
    annotation << "BEGIN: edgeMapChunked() Init";
    ...
    par_for(0, indices.size(), pbbslib::kSequentialForThreshold, [&](size_t i) {
        ...
        auto neighbors = (fl & in_edges) ? G.get_vertex(vtx_id).in_neighbors()
                                         : G.get_vertex(vtx_id).out_neighbors();
        par_for(0, num_vertex_blocks, pbbslib::kSequentialForThreshold,
                [&](size_t j) {
                    size_t block_deg = neighbors.block_degree(j);
                    // assert(block_deg <= PARALLEL_DEGREE); // only for
                    // compressed
                    blocks[vtx_off + j] =
                        block(i, j); // j-th block of the i-th vertex.
                    degrees[vtx_off + j] = block_deg;
    ...
    size_t edge_block_size_guess =
        pbbs::num_blocks(outEdgeCount, num_workers() << 3);
...
#ifdef ACCESS_OBSERVER
    annotation << "BEGIN: edgeMapChunked() Process";
    access_observer.write(
        annotation.str()
    );
    std::stringstream().swap(annotation);
        ...
        [&](size_t group_id) {
            ...
            if (our_start != our_end && our_start != num_blocks) {
                for (size_t work_id = our_start; work_id < our_end; work_id++) {
                    // 1. before starting next work block check whether we need
                    // to reallocate the output block. This guarantees that
                    // there is enough space in the output block even if all
                    // items in the work block are written out
                    em_data_block *out_block =
                        our_emhelper.get_block_and_offset_for_group(group_id);
                    size_t offset = out_block->block_size;
                    auto out_block_data =
                        (std::tuple<uintE, data> *)out_block->data;
                    ...
                    auto neighbors = (fl & in_edges)
                                         ...
                                         : G.get_vertex(vtx_id).out_neighbors();
                    size_t num_in =
                        neighbors.decode_block(offset, block_num, f, g);
    ...
    auto block_offsets = pbbs::sequence<size_t>(
    ...
    if (output_size > 0) {
        ...
        parallel_for(
            ...
            [&](size_t block_id) {
                ...
                std::tuple<uintE, data> *block_data =
                    (std::tuple<uintE, data> *)block->data;
...
//#ifdef ACCESS_OBSERVER
//        annotation << "END: edgeMapChunked(),"
//                   << "NextAddress:"
//                   << ret.s
//                   << ","
//                   << "NextSize:"
//                   << (sizeof(ret.s[0]) * output_size);
//        access_observer.write(
//            annotation.str()
//        );
//        std::stringstream().swap(annotation);
