...
// Returns u
template <class Seq> inline size_t RelabelIds(Seq &ids) {
...
std::pair<edge *, size_t> fetch_intercluster_small(Graph &GA, C &clusters,
                                                   size_t num_clusters) {
    ...
    KV empty = std::make_tuple(std::make_tuple(UINT_E_MAX, UINT_E_MAX),
                               pbbslib::empty());
    ...
    auto map_f = [&](const uintE &src, const uintE &ngh, const W &w) {
        ...
        if (c_src < c_ngh) {
            edge_table.insert(std::make_tuple(std::make_tuple(c_src, c_ngh),
                                              pbbslib::empty()));
        }
    };
    par_for(0, n, 1,
    ...
    edge_table.del();
    ins_t.stop();
    debug(ins_t.reportTotal("insertion time"););
...
template <class Graph, class C>
std::pair<edge *, size_t> fetch_intercluster_te(Graph &GA, C &clusters,
                                                size_t num_clusters) {
...
template <class Graph, class C>
std::pair<edge *, size_t> fetch_intercluster(Graph &GA, C &clusters,
                                             size_t num_clusters) {
    using K = std::tuple<uintE, uintE>;
    using V = pbbslib::empty;
    using KV = std::tuple<K, V>;
    ...
    auto map_f = [&](const uintE &src, const uintE &ngh, const W &w) {
        ...
        if (c_src < c_ngh) {
            edge_table.insert_check(
                std::make_tuple(std::make_tuple(c_src, c_ngh),
                                pbbslib::empty()),
                &abort);
        }
    };
    parallel_for(
        0, n, [&](size_t i) { GA.get_vertex(i).out_neighbors().map(map_f); },
        1);
    if (abort) {
...
inline std::tuple<symmetric_graph<symmetric_vertex, pbbslib::empty>,
...
contract(Graph &GA, sequence<uintE> &clusters, size_t num_clusters) {
    ...
    auto GC = sym_graph_from_edges<pbbslib::empty>(/* edges = */ sym_edges,
                                                   /* n = */ num_ns_clusters);
