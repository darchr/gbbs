...
parse_unweighted_graph(const char *fname, bool mmap, char *bytes,
                       size_t bytes_size) {
...
read_unweighted_symmetric_graph(const char *fname, bool mmap, char *bytes,
                                size_t bytes_size) {
    size_t n, m;
    uintT *offsets;
    uintE *edges;
    std::tie(n, m, offsets, edges) =
        parse_unweighted_graph(fname, mmap, bytes, bytes_size);
...
read_unweighted_asymmetric_graph(const char *fname, bool mmap, char *bytes,
    ...
    pbbslib::integer_sort_inplace(
        temp_seq.slice(), [&](const intPair &p) { return p.first; },
        pbbs::log2_up(n));
    ...
    return asymmetric_graph<asymmetric_vertex, pbbs::empty>(
        ...
        [=]() { pbbslib::free_arrays(v_data, v_in_data, inEdges, edges); },
        (std::tuple<uintE, pbbs::empty> *)edges,
        (std::tuple<uintE, pbbs::empty> *)inEdges);
}
...
std::tuple<char *, size_t> parse_compressed_graph(const char *fname, bool mmap,
    ...
    if (mmap) {
        ...
        if (mmapcopy) {
            debug(std::cout
                      << "# Copying compressed graph due to mmapcopy being set."
                      << "\n";);
            char *next_bytes = pbbslib::new_array_no_init<char>(bytes_size);
            par_for(0, bytes_size, pbbslib::kSequentialForThreshold,
                    [&](size_t i) { next_bytes[i] = bytes[i]; });
