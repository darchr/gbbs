...
template <typename T> using sequence = pbbs::sequence<T>;

// Alias template so that range is exposed w/o namespacing
template <typename T> using range = pbbs::range<T>;
...
template <typename ET> inline bool CAS(ET *ptr, const ET oldv, const ET newv) {
...
template <class ET> inline ET xadd(ET *variable, ET value) {
    if (sizeof(ET) == 8) {
...
template <typename ET> inline bool write_min(ET *a, ET b) {
    return pbbs::write_min<ET>(a, b, std::less<ET>());
}

template <typename ET> inline bool write_max(ET *a, ET b) {
...
template <class T> using minm = pbbs::minm<T>;

template <class T> using maxm = pbbs::maxm<T>;

template <class T> using addm = pbbs::addm<T>;
...
template <class T> inline pbbs::range<T *> make_sequence(T *A, size_t n) {
    return pbbs::range<T *>(A, A + n);
}

// Scans the input sequence using the addm monoid.
//
// This computes in-place an exclusive prefix sum on the input sequence, that
// is,
...
inline auto scan_add_inplace(
    ...
    return pbbs::scan_inplace(std::forward<In_Seq>(In), pbbs::addm<T>(), fl,
                              tmp);
}

template <class Seq>
inline auto reduce_add(Seq const &I, flags fl = no_flag) ->
...
template <class Seq>
inline auto reduce_max(Seq const &I, flags fl = no_flag) ->
    typename Seq::value_type {
    using T = typename Seq::value_type;
...
template <class Seq>
inline auto reduce_min(Seq const &I, flags fl = no_flag) ->
    typename Seq::value_type {
    using T = typename Seq::value_type;
...
template <class Seq>
inline auto reduce_xor(Seq const &I, flags fl = no_flag) ->
    typename Seq::value_type {
    using T = typename Seq::value_type;
...
size_t pack_index_out(Bool_Seq const &Fl, Out_Seq &&Out, flags fl = no_flag) {
    ...
    return pbbs::pack_out(pbbs::delayed_seq<Idx_Type>(Fl.size(), identity), Fl,
...
template <class T> void free_arrays(T *first) { pbbs::free_array(first); }

template <class T, typename... Args> void free_arrays(T *first, Args... args) {
    pbbs::free_array(first);
    free_arrays(args...);
}

template <class Idx_Type, class D, class F>
inline pbbs::sequence<std::tuple<Idx_Type, D>>
pack_index_and_data(F &f, size_t size, flags fl = no_flag) {
    auto id_seq =
        pbbslib::make_sequence<std::tuple<Idx_Type, D>>(size, [&](size_t i) {
            return std::make_tuple((Idx_Type)i, std::get<1>(f[i]));
        });
    auto flgs_seq = pbbslib::make_sequence<bool>(
...
template <class E, class I, class P> struct filter_iter {
    I &iter;
    P &pred;
    E cur_val;
...
template <class A, class B> inline int t_to_stringlen(std::pair<A, B> a) {
    return t_to_stringlen(a.first) + t_to_stringlen(a.second) + 1;
}

template <class A, class B> inline int t_to_stringlen(std::tuple<A, B> a) {
...
template <class TSeq> sequence<char> sequence_to_string(TSeq const &T) {
    size_t n = T.size();
