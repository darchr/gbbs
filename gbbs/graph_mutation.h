...
filter_graph(Graph &G, P &pred) {
    ...
    parallel_for(
        ...
        [&](size_t i) {
            ...
            auto out_f = [&](uintE j) {
                return static_cast<int>(pred(i, u_out_nghs.get_neighbor(j),
                                             u_out_nghs.get_weight(j)));
            };
    ...
    parallel_for(
        ...
        [&](size_t i) {
            ...
            if (d > 0) {
                ...
                pbbslib::filter_out(n_im, pbbslib::make_sequence(dir_nghs, d),
                                    pred_c, pbbslib::no_flag);
            }
        },
        1);
    ...
    return std::make_tuple(G.num_vertices(), outEdgeCount, out_vdata,
                           out_edge_arr);
}
    ...
    parallel_for(
        ...
        [&](size_t i) {
            ...
            if (new_deg > 0) {
                ...
                auto f_it = pbbslib::make_filter_iter<std::tuple<uintE, W>>(
                    iter, app_pred);
...
inline edge_array<typename Graph::weight_type>
filter_edges(Graph &G, P &pred, const flags fl = 0) {
    ...
    auto red_f =
        [](const std::tuple<uintT, uintT> &l, const std::tuple<uintT, uintT> &r)
    ...
    parallel_for(
        ...
        [&](size_t i) {
            auto res =
                G.get_vertex(i).out_neighbors().reduce(map_f, red_monoid);
            if (std::get<0>(res) > 0 || std::get<1>(res) > 0) {
                ...
                vtx_offs[i] =
                    std::make_tuple(std::get<0>(res), std::get<1>(res), 0);
            }
        },
        1);
    reduce_t.stop();
    reduce_t.reportTotal("reduce time");
    ...
    parallel_for(
        ...
        [&](size_t i) {
            ...
            size_t n_one =
                std::get<0>(vtx_offs[i + 1]) - std::get<0>(vtx_offs[i]);
            size_t n_two = std::get<1>(vtx_offs[i + 1]) - off;
            size_t n_to_pack = n_one + n_two;
            if (n_to_pack > 0) {
                std::tuple<uintE, W> *tmp_v =
                    tmp.begin() + std::get<2>(vtx_offs[i]);
                ...
                if (n_two > 0 && !(fl & no_output)) {
                    G.get_vertex(i).out_neighbors().filter(pred_two, out_f,
                                                           tmp_v);
                }
        ...
        1);
    auto degree_imap = pbbslib::make_sequence<size_t>(
...
template <class Graph, class P>
inline edge_array<typename Graph::weight_type> filter_all_edges(Graph &G, P &p,
                                                                flags fl = 0) {
    using W = typename Graph::weight_type;
    using edge = std::tuple<uintE, uintE, W>;
    size_t n = G.n;
    ...
    pbbslib::scan_inplace(offs.slice(),
...
edge_array<typename Graph::weight_type> sample_edges(Graph &G, P &pred) {
    ...
    pbbslib::scan_inplace(vtx_offs.slice(),
                          pbbslib::make_monoid(scan_f, std::make_tuple(0, 0)));
    ...
    // 2. Filter edges into output arr.
    {
        parallel_for(
            ...
            [&](size_t i) {
                ...
                if (n_to_pack > 0) {
                    std::tuple<uintE, W> *tmp_v =
                        tmp.begin() + std::get<1>(vtx_offs[i]);
                    auto out_f = [&](size_t j, const std::tuple<uintE, W> &nw) {
                        output_arr[off + j] = std::make_tuple(
                            i, std::get<0>(nw), std::get<1>(nw));
                    };
...
inline vertexSubsetData<uintE> packEdges(Graph &G, vertexSubset &vs, P &p,
    ...
    parallel_for(0, m, [&](size_t i) {
        uintE v = vs.vtx(i);
        space[i] =
            G.get_vertex(v).out_neighbors().calculateTemporarySpaceBytes();
    });
    space[m] = 0;
    size_t total_space = pbbslib::scan_add_inplace(space);
    uint8_t *tmp = nullptr;
